...
const requestInterface = {
  ...
  getUserInfo: 'v4/profile/portrait_get',
};
...
const getUserInfo = async userId => {
  checkAdminUserSig();
  const url = getPullStreamUrl(Protocol, Domain, requestInterface.getUserInfo, SdkAppId, Identifier, userInfo.UserSig);
  try {
    const response = await sendRequest(url, RequestMethods.POST, {
      "To_Account":[userId],
      "TagList":
...
module.exports = { fetchLiveList, destroyRoom, getRoomInfo, getUserInfo };
